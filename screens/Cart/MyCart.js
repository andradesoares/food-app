...
const MyCart = ({ navigation }) => {
  ...
  const updateQuantity = (newQty, id) => {
    const newCartList = myCartList.map((cl) => (cl.id === id ? { ...cl, qty: newQty } : cl));
      ...
      <SwipeListView
          ...
          return (
            ...
            >
              ...
              >
                ...
                <Text style={{ color: COLORS.primary, ...FONTS.body3 }}>{data.item.price}</Text>
              </View>
              <StepperInput
                containerStyle={{
                  height: 50,
                ...
                onMinus={() => {
                  data.item.qty > 1 ? updateQuantity(data.item.qty - 1, data.item.id) : null;
                }}
                value={data.item.qty}
              />
            </View>
        ...
        renderHiddenItem={(data, rowMap) => (
          <IconButton
            ...
            onPress={() => removeItem(data.item.id)}
          />
        )}
      />
      <FooterTotal
